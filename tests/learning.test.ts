...
beforeEach(() => {
  localStorage.clear()
  useLearning.setState(emptyData())
...
describe('review scheduling and selection', () => {
  it('uses calendar-day intervals and resets streak on hard and again', () => {
    ...
    for (const days of [7, 14, 30, 30]) {
      progress = applyRating(progress, 'good', now)
      expect(progress.nextReviewAt).toBe(addDays(new Date(now), days).toISOString())
    }
    ...
    expect(applyRating(progress, 'again', now).nextReviewAt).toBe(
      addDays(new Date(now), 1).toISOString(),
    )
  })
  ...
  it('combines text, metadata and progress filters', () => {
    const reactQuestion = questions.find((q) => q.category === 'react')!
    const p = { [reactQuestion.id]: { ...blankProgress(reactQuestion.id), favorite: true } }
    expect(
      filterQuestions(questions, new URLSearchParams('category=react&status=favorite'), p).map(
        (q) => q.id,
      ),
    ).toEqual([reactQuestion.id])
...
describe('persistent learning records', () => {
  ...
  it('retains chronological ratings when an earlier report changes', () => {
    ...
    useLearning.getState().ensureDetail(questions[0])
    useLearning.getState().rate({ kind: 'detail', questionId: 1 }, 'hard')
  ...
  it('imports progress-only legacy backups without inventing history and preserves the rating baseline', () => {
    ...
    const imported = decodeBackup(
      JSON.stringify({ version: 1, state: { progress: { 1: p }, theme: 'light' } }),
    )
    expect(imported.records).toEqual([])
    // 旧的 light/dark 取值统一迁移到默认的 Vitesse 主题。
    expect(imported.theme).toBe('vitesse-light')
    useLearning.getState().importData(imported)
    useLearning.getState().ensureDetail(questions[0])
    useLearning.getState().rate({ kind: 'detail', questionId: 1 }, 'good')
    expect(useLearning.getState().progress[1].goodStreak).toBe(3)
    expect(useLearning.getState().records).toHaveLength(1)
  })
  it('normalizes version 2 themes and exports version 3 backups', () => {
    const legacy = decodeBackup(
      JSON.stringify({ version: 2, state: { ...emptyData(), theme: 'dark' } }),
    )
    expect(legacy.theme).toBe('vitesse-dark')
    useLearning.getState().setTheme('catppuccin-mocha')
    const payload = JSON.parse(exportData())
    expect(payload.version).toBe(3)
    expect(payload.state.theme).toBe('catppuccin-mocha')
    expect(decodeBackup(JSON.stringify(payload)).theme).toBe('catppuccin-mocha')
  })
  it('backs up malformed storage and reports quota failures', () => {
    localStorage.setItem(STORAGE_KEY, '{invalid')
    expect(safeStorage.getItem(STORAGE_KEY)).toBeNull()
    expect(Object.keys(localStorage).some((key) => key.startsWith(`${STORAGE_KEY}:corrupt:`))).toBe(
      true,
    )
  ...
  it('rejects invalid import and duplicate record IDs before replacing state', () => {
    expect(() => decodeBackup('{bad')).toThrow(SyntaxError)
    expect(() => decodeBackup(JSON.stringify({ version: 99, state: {} }))).toThrow('版本')
    useLearning.getState().ensureDetail(questions[0])
    useLearning.getState().rate({ kind: 'detail', questionId: 1 }, 'good')
    const payload = JSON.parse(exportData())
    payload.state.records.push(payload.state.records[0])
  ...
  it('groups records by stored local day, excluding unanswered and skipped attempts', () => {
    useLearning.getState().start('mock', questions.slice(0, 5), 10)
    useLearning.getState().patchItem({ kind: 'mock' }, { status: 'answered' })
    useLearning.getState().finish('mock')
    ...
    expect(
      dailyStats(
        [{ ...records[0], localDate: localDate(new Date(2026, 8, 14, 23, 59)) }],
        2,
        midnight,
      ).map((d) => d.count),
    ).toEqual([1, 0])
