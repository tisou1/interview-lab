...
const question: Question = {
  ...questions[0],
  promptHtml: '<p>独立思考题干</p>',
  answerSections: [
    { id: 'test-answer', title: '核心答案', html: '<p>核心答案正文</p>' },
    { id: 'test-follow', title: '追问', html: '<p>追问正文</p>' },
  ],
}
function Workspace() {
  const item = useLearning((s) => s.details[1])
  return (
    <MemoryRouter>
      <QuestionWorkspace
        question={question}
        item={item}
        context={{ kind: 'detail', questionId: 1 }}
      />
    </MemoryRouter>
  )
}
...
describe('active recall interaction', () => {
  ...
  it('does not activate global shortcuts inside a draft', () => {
    render(<Workspace />)
    const draft = screen.getByRole('textbox', { name: '回答草稿' })
    fireEvent.change(draft, { target: { value: 'F R 123' } })
    fireEvent.keyDown(draft, { key: 'f' })
    fireEvent.keyDown(draft, { code: 'Space', key: ' ' })
  ...
  it('does not render answers while a mock is active, even from the question route', () => {
    ...
    render(<Workspace />)
    expect(screen.queryByRole('button', { name: /查看参考答案/ })).not.toBeInTheDocument()
