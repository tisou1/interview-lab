...
```sh
corepack pnpm content     # 单独校验并生成题库
corepack pnpm typecheck
corepack pnpm test
corepack pnpm build      # 转换、类型检查、构建、bundle budget
corepack pnpm preview
corepack pnpm exec playwright install chromium
corepack pnpm test:e2e
corepack pnpm deps:check  # 项目内 taze 检查所有直接依赖；pnpm 按要求保留 11.x
```
...
采用 BrowserRouter，需要静态托管支持 SPA 回退。将 `dist/` 部署到站点根路径，并将应用路由回退到 `index.html`。已提供 `public/_redirects` 和 [Nginx 示例](deploy/nginx.conf)。

验收时直接访问并刷新 `/questions/1`、`/mock/report/示例ID`；第二个地址即使没有对应本地报告，也应显示网站内的空状态。实际静态资源缺失应返回 404。当前配置面向根路径部署；子目录部署需同时调整 Vite `base` 和 BrowserRouter `basename`。
