...
现有题库包含编号 1–150 的题目，另有模拟卷、评分模板与参考资料。其中 1–115 为初版题，116–150 为现代 React、浏览器性能与工程化方向的扩展题。附录原文保留，不将附录当成新的独立题目。网站模拟面试按配置抽题。
...
答案区统一用 Markdown 书写，不要写 HTML 标签：

- `<details>` 与 `<summary>` 之后必须留一个空行，否则后续正文会被当成原始 HTML 而不做 Markdown 解析。
- 不要用 `<div>` 包装；解析器会解包 `div`，但新内容无需依赖该行为。
- 代码块一律使用围栏写法（` ```js `），代码内部的空行是安全的；旧版 HTML 代码块在遇到内部空行时会被 CommonMark 的 HTML 块规则截断，导致同一段代码被渲染成多块。
- 小节标题使用固定词表，不要自创同义词：核心答案（无标题首段）、`原理与示例`、`边界与易错点`、`追问`、`参考实现`、`复杂度与取舍`、`设计框架`、`设计要点与边界`、`排查步骤`、`评分点`。
- 首段是「核心答案」，先给结论；短题可以省略部分小节，但必须保留核心答案，并至少含「边界与易错点」或「追问」之一。
- 答案小标题不要写 `id` 属性，锚点由站点自行生成。
- 答案区之后除分隔线与空行外不得有其他内容，否则解析报「答案区之后有未归属内容」。

`tests/parser.test.ts` 会对小节命名、空小节与代码块结构做全库断言，新增或修改题目后直接运行该测试即可回归。
...
`buildContent()` 接收可选的 `onProgress(label, current, total)`，依次上报「读取源文件」「解析题目」「渲染学习路线」「写入生成数据」四个阶段。`pnpm dev` 与 `pnpm content` 把这些回调渲染成 `cli-progress` 进度条（颜色由 `picocolors` 提供，两者都是仅用于构建的开发依赖，不进入产物）；非 TTY 环境只打印一行结果，不会输出控制字符。

## 校验与维护

必检：唯一正整数 ID、非空标题、元数据枚举、非空 tags、正数预计时间、有效且非空的答案区。错误示例：`fe/题库.md:42: Q116 难度无效`。

`scripts/annotate-legacy.mjs` 是初始 115 道题（编号 1–115）的幂等迁移脚本，已执行。后续维护以 Markdown 内的元数据为准，不需要再次运行迁移。难度和预计时间是可编辑的学习建议，不是客观难度评分。
