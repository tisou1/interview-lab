...
export const categories = ['javascript', 'react', 'browser', 'typescript', 'engineering', 'coding', 'architecture']
const reader = unified().use(remarkParse).use(remarkGfm)
const renderer = unified().use(remarkParse).use(remarkGfm)
  .use(remarkRehype, { allowDangerousHtml: true }).use(rehypeRaw)
  .use(rehypeSanitize, { ...defaultSchema, attributes: { ...defaultSchema.attributes, code: [['className', /^language-./]], '*': [] } })
...
function safeTree(markdown) {
  ...
  function links(node) {
    ...
    node.children?.forEach(links)
  }
  links(tree)
  return tree
}

export function renderMarkdown(markdown) { return html(safeTree(markdown).children) }
...
export function parseQuestions(markdown, filename = 'questions.md') {
  ...
  for (let index = 0; index < boundaries.length; index++) {
    ...
    const fail = message => { throw new Error(`${filename}:${heading.position.start.line}: Q${id} ${message}`) }
    if (!Number.isSafeInteger(id) || id < 1) fail('编号必须为正整数')
    if (!title) fail('题目标题不能为空')
    if (seen.has(id)) fail('题目编号重复')
    seen.add(id)
    const body = markdown.slice(heading.position.end.offset, boundaries[index + 1]?.position.start.offset ?? markdown.length)
    const metadataMatches = [...body.matchAll(/<!--\s*question:\s*([\s\S]*?)-->/g)]
    if (metadataMatches.length !== 1) fail('每道题需要且仅需要一份 question 元数据')
    let metadata
    try { metadata = JSON.parse(metadataMatches[0][1]) } catch { fail('question 元数据不是有效 JSON') }
    if (!metadata || typeof metadata !== 'object' || Array.isArray(metadata)) fail('元数据必须为对象')
    ...
    if (!Array.isArray(metadata.tags) || !metadata.tags.length || metadata.tags.some(tag => typeof tag !== 'string' || !tag.trim())) fail('tags 必须为非空字符串数组')
    if (!Number.isFinite(metadata.estimatedMinutes) || metadata.estimatedMinutes <= 0) fail('预计用时必须大于 0')
    const bodyAst = reader.parse(body)
    const details = bodyAst.children.find(node => node.type === 'html' && /^\s*<details(?:\s[^>]*)?>/i.test(node.value))
    ...
    if (end < 0 || (remainder.match(/<details(?:\s[^>]*)?>/g) ?? []).length !== 1 || (remainder.match(/<\/details>/g) ?? []).length !== 1) fail('答案区 details 结构不完整或嵌套')
    if (body.slice(start + end + 10).replace(/\s|---/g, '')) fail('答案区之后有未归属内容')
    const answer = remainder.slice(open[0].length, end)
    ...
    let nodes = []
    const flush = () => {
      if (!nodes.some(node => node.type !== 'text' || node.value.trim())) { nodes = []; return }
      answerSections.push({ id: `q${id}-section-${answerSections.length}`, title: sectionTitle, html: html(nodes) })
      nodes = []
    }
    for (const node of flattened) {
      if (node.type === 'element' && /^h[2-4]$/.test(node.tagName)) { flush(); sectionTitle = plain(node) || '解析' }
      else nodes.push(node)
    }
    flush()
    if (!answerSections.length || !answerSections.some(s => s.html.replace(/<[^>]*>|\s/g, ''))) fail('参考答案不能为空')
    const prompt = body.slice(0, start).replace(metadataMatches[0][0], '').trim()
    result.push({ id, slug: `question-${id}`, title, category: metadata.category, type: metadata.type, difficulty: metadata.difficulty, tags: metadata.tags, estimatedMinutes: metadata.estimatedMinutes,
      promptHtml: renderMarkdown(prompt), answerSections,
...
export function buildContent() {
  ...
  const questions = parseQuestions(fs.readFileSync(sourceFiles[0], 'utf8'), 'fe/' + path.basename(sourceFiles[0]))
  ...
  write('index', questions.map(({ promptHtml, answerSections, ...question }) => ({ ...question, searchText: `${question.title} ${question.tags.join(' ')}`.toLowerCase() })))
  for (const category of categories) write(category, questions.filter(question => question.category === category))
