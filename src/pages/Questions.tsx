...
export default function Questions({ review = false }: { review?: boolean }) {
  ...
  return <>
    ...
    <section className={`${styles.card} ${styles.filters}`} aria-label="题库筛选"><label className={styles.searchField}><Icon name="search" size={18}/><input aria-label="搜索题目" placeholder="搜索题目、关键词或编号，例如：闭包、React…" value={query} onChange={e => setQuery(e.target.value)}/>{query && <button className={styles.iconButton} aria-label="清空搜索" onClick={() => { setQuery(''); update('q', '') }}><Icon name="close" size={16}/></button>}</label>
      <div className={styles.filterRow}>{([['category', '专题', categoryNames], ['type', '题型', typeNames], ['difficulty', '难度', difficultyNames]] as const).map(([key, label, options]) => <label className={styles.field} key={key}>{label}<AppSelect label={label} value={params.get(key) ?? ''} onValueChange={value => update(key, value)} options={[{ value: '', label: `全部${label}` }, ...Object.entries(options).map(([value, name]) => ({ value, label: name }))]}/></label>)}<label className={styles.field}>标签<AppSelect label="标签" value={params.get('tag') ?? ''} onValueChange={value => update('tag', value)} options={[{ value: '', label: '全部标签' }, ...tags.map(tag => ({ value: tag, label: tag }))]}/></label></div>
    </section>
    <div className={styles.sectionHeading}><span className={styles.muted}>找到 <strong>{result.length}</strong> 道题{query && ` · “${query}”`}</span><div className={styles.actions}><AppSelect label="排序" value={params.get('sort') ?? ''} onValueChange={value => update('sort', value)} options={[{ value: '', label: '按题目编号' }, { value: 'difficulty', label: '按难度' }, { value: 'recent', label: '按最近复习' }]}/><button className={styles.iconButton} onClick={() => update('view', params.get('view') === 'compact' ? '' : 'compact')} aria-pressed={params.get('view') === 'compact'} aria-label="切换紧凑视图"><Icon name="menu"/></button>{params.toString() && <button className={styles.tab} onClick={() => { setQuery(''); setParams({}); setFilters('') }}>重置</button>}{!params.toString() && previousFilters.current && <button className={styles.tab} onClick={() => setParams(previousFilters.current)}>恢复上次筛选</button>}</div></div>
