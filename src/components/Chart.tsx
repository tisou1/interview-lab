...
export function BarChart({
  ...
  const width = 640,
    height = horizontal ? Math.max(180, points.length * 42) : 190
  ...
  return (
    <section className={styles.chartCard} aria-labelledby={id}>
      <div className={styles.sectionHeading}>
        ...
        <span className={styles.chartTooltip} aria-live="polite">
          {active !== null ? `${points[active].label} · ${points[active].value}${unit}` : ''}
        </span>
      </div>
      <svg
        ...
        role="img"
        aria-label={`${title}，${total === 0 ? '暂无记录' : `共 ${points.length} 项数据，详情见下表`}`}
      >
        {!horizontal &&
          [0, 0.5, 1].map((fraction) => (
            <g key={fraction}>
              ...
              />
              <text x="0" y={height - fraction * (height - 20) + 4}>
                {Math.round(max * fraction * 10) / 10}
              </text>
            </g>
          ))}
        {points.map((point, index) => {
          ...
          return (
            ...
            >
              ...
              {horizontal && (
                <rect x={x} y={y} width={width - 195} height={h} rx="4" className={styles.track} />
              )}
              <m.rect
                x={x}
                y={y}
                ...
                height={Math.max(h, 1)}
                rx="3"
                fill={point.color ?? 'var(--chart)'}
                initial={
                  reduced
                    ? false
                    : { opacity: 0, scaleY: horizontal ? 1 : 0.01, scaleX: horizontal ? 0.01 : 1 }
                }
                animate={{
              ...
              />
              {(horizontal || points.length < 15 || index % Math.ceil(points.length / 7) === 0) && (
                <text
                  x={horizontal ? 0 : x + w / 2}
                  y={horizontal ? y + 12 : height + 24}
