...
const icons: Record<string, ReactNode> = {
  ...
  book: <><path d="M12 5c-3-2-7-2-9-1v15c3-1 6-1 9 1 3-2 6-2 9-1V4c-2-1-6-1-9 1Z"/><path d="M12 5v15"/></>,
  play: <path d="m8 4 13 8-13 8Z"/>,
  review: <><path d="M4 10a8 8 0 1 1 1 8M4 4v6h6"/><path d="M12 8v5l3 2"/></>,
  mock: <><rect x="4" y="5" width="16" height="16" rx="2"/><path d="M9 5V3h6v2M8 11h8M8 16h5"/></>,
...
export function Icon({ name, size = 20 }: { name: string; size?: number }) { return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{icons[name] ?? icons.code}</svg> }
export function PageHeader({ eyebrow, title, description, action }: { eyebrow: string; title: string; description?: string; action?: ReactNode }) { return <HeaderEntrance className={styles.pageHeader}><div><div className={styles.eyebrow}>{eyebrow}</div><h1>{title}</h1>{description && <p>{description}</p>}</div>{action}</HeaderEntrance> }
export function Badge({ mastery = 'new' }: { mastery?: Mastery }) { return <span className={`${styles.badge} ${styles[mastery]}`}><span className={styles.dot}/>{masteryNames[mastery]}</span> }
export function Empty({ title, children, to, label }: { title: string; children?: ReactNode; to?: string; label?: string }) { return <div className={styles.empty}><Icon name="book" size={30}/><h3>{title}</h3><p>{children}</p>{to && <Link className={styles.primary} to={to}>{label ?? '去题库看看'}<Icon name="arrow" size={16}/></Link>}</div> }
export function HtmlContent({ html }: { html: string }) { return <div className={styles.prose} dangerouslySetInnerHTML={{ __html: html }}/> }
export function useQuestion(id: number) {
  const [result, setResult] = useState<{ id: number; question?: Question; error?: boolean }>({ id })
  useEffect(() => { let active = true; loadQuestion(id).then(question => { if (active) setResult({ id, question }) }).catch(() => { if (active) setResult({ id, error: true }) }); return () => { active = false } }, [id])
